...
public class AuthService implements IAuthService {
    ...
    @Override
    public String createVerifyCode(UserMessageEntity userMessageEntity) {
        String code = redisService.getValue(userMessageEntity.getOpenId());
        if(code != null){
            return code;
        }
        // 生成验证码
...
//        Token 并返回
    ...
    // 也就是拿到荷载部分所有的键值对
    private Claims decode(String jwtToken) {
        // 得到 DefaultJwtParser
        return Jwts.parser()
                // 设置签名的秘钥
